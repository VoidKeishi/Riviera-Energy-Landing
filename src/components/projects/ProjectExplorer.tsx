    ...
    <>
      ...
      <div className="md:hidden space-y-3">
        {projects.map((project) => (
          <div
            key={project.title}
            className="rounded-xl border border-[var(--color-grey-100)] bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)] p-4"
          >
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[11px] font-sans text-[var(--color-text-muted)] flex items-center gap-1">
            ...
            {/* Title */}
            <h3 className="font-display text-base font-bold text-[var(--color-text-primary)] leading-tight mb-2">
              {project.title}
            </h3>

            {/* Description */}
            <p className="text-xs font-sans text-[var(--color-text-body)] leading-relaxed mb-3">
              {renderHighlightedText(project.description, project.highlights)}
            </p>
            ...
            <div className="flex flex-wrap gap-1">
              {project.services.map((service) => (
                <span
                  key={service}
                  className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-sans font-medium bg-[var(--color-off-white)] text-[var(--color-text-muted)] border border-[var(--color-grey-200)]"
                >
                  {service}
                </span>
              ))}
      ...
      {/* Desktop: interactive map — only mount when md+ to avoid Leaflet zero-size container issues */}
      {isDesktop && <div
        className="relative rounded-xl overflow-hidden border border-[var(--color-grey-100)] shadow-[0_4px_24px_-4px_rgba(8,1,69,0.08)] h-[500px] lg:h-[560px] bg-[#e8ecf0]"
        role="application"
          ...
          {selectedProject && (
            <div className="absolute bottom-4 left-4 pointer-events-auto max-w-md">
              <div
                ...
                className="bg-white/92 backdrop-blur-md rounded-lg border border-[var(--color-grey-100)] shadow-lg p-4"
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-xs font-sans text-[var(--color-text-muted)] flex items-center gap-1">
                    <svg className="w-3 h-3 text-[var(--color-energy-mid)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                ...
                {/* Title */}
                <h3 className="font-display text-lg font-bold text-[var(--color-text-primary)] leading-tight mb-1.5">
                  {selectedProject.title}
                </h3>

                {/* Description with highlighted keywords */}
                <p className="text-sm font-sans text-[var(--color-text-body)] leading-relaxed mb-2.5">
                ...
                <div className="flex flex-wrap gap-1">
                  {selectedProject.services.map((service) => (
                    <span
                      key={service}
                      className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-sans font-medium bg-[var(--color-off-white)] text-[var(--color-text-muted)] border border-[var(--color-grey-200)]"
                    >
                      {service}
                    </span>
                  ))}
                ...
                {projects.map((project) => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        w-full flex items-center gap-2 px-3 py-1.5 text-left text-xs font-sans
